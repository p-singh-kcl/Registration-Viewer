...
def run_viewer(t1w, t1c, brain_mask,
               ...
               scan1_label="T1w", scan2_label="T1c",
               window_title="Registration Quality"):
    ...
    fig1.subplots_adjust(left=0.02, right=0.98, top=0.88, bottom=0.18,
                          wspace=0.05)
    fig1.suptitle(f"{window_title} — Intensity", color="white",
                  fontsize=14, fontweight="bold")

    for ax in [ax1, ax2, ax3]:
        ax.set_facecolor("black"); ax.set_xticks([]); ax.set_yticks([])

    ax1.set_title(scan1_label, color="white", fontsize=11, pad=4)
    ax2.set_title(scan2_label, color="white", fontsize=11, pad=4)
    ax3.set_title(f"{scan1_label} + structural Δ", color="white", fontsize=11, pad=4)
    ...
    if has_label_window:
        ...
        fig2.subplots_adjust(left=0.02, right=0.98, top=0.88, bottom=0.18,
                              wspace=0.05)
        fig2.suptitle(f"{window_title} — Disagreement Analysis",
        ...
        t1w_title = (f"{scan1_label}: GT vs Pred" if has_pred_t1w
                     else f"{scan1_label} + GT boundaries")
        t1c_title = (f"{scan2_label}: GT vs Pred" if has_pred_t1c
                     else f"{scan2_label} + GT boundaries")
        cross_title = (f"Pred {scan1_label} vs Pred {scan2_label}"
                       if has_pred_cross else "(need both preds)")
    ...
    ax_z.set_title(f"{scan1_label}  |  {scan2_label}   (click main window to inspect)",
    ...
    if has_label_window:
        ...
        ax_lz.set_title(f"{scan1_label}+labels  |  {scan2_label}+labels  |  "
                        f"Disagreement   (click label window)",
    ...
    if has_error_corr:
        ...
        fig_ec.subplots_adjust(left=0.02, right=0.98, top=0.88, bottom=0.18,
                                wspace=0.05)
        fig_ec.suptitle(
            f"{window_title} — Error Correlation   "
            "Red=Pred Error  Blue=Struct Δ  Purple=Both",
            color="white", fontsize=12, fontweight="bold")

        for ax in [ax_ec1, ax_ec2]:
            ax.set_facecolor("black"); ax.set_xticks([]); ax.set_yticks([])

        ax_ec1.set_title(scan1_label if has_pred_t1w else f"{scan1_label} (no pred)",
                         color="#cc88ff", fontsize=11, pad=4)
        ax_ec2.set_title(scan2_label if has_pred_t1c else f"{scan2_label} (no pred)",
                         color="#cc88ff", fontsize=11, pad=4)
    ...
    if has_error_corr:
        ...
        ax_ecz.set_title(
            f"{scan1_label} err | {scan2_label} err  (click error correlation window)",
    ...
    def update_intensity_zoom(sl_t1w, sl_t1c, sl_diff, h, w):
        ...
        ax_z.set_title(
            f"{scan1_label} | {scan2_label} | Δ   "
            f"CosSim:{local_cos:.3f}  SSIM:{local_ssim:.3f}  [{2*r}px]",
            color="#ffcc00", fontsize=11
        )
        fig_z.canvas.draw_idle()
    ...
    def update_label_zoom(sl_t1w, sl_t1c, sl_mask,
        ...
        ax_lz.set_title(
            f"{scan1_label} err | {scan2_label} err | GT disagree   [{2*r}px]",
            color="#ffcc00", fontsize=10
        )
    ...
    def update_ec_zoom(sl_t1w, sl_t1c, sl_diff, sl_mask, axis, idx, h, w,
        ...
        ax_ecz.set_title(
            f"{scan1_label} err | {scan2_label} err   [{2*r}px]",
            color="#cc88ff", fontsize=10)
...
def launch_file_picker():
    ...
    root = tk.Tk()
    root.title("Registration Viewer — Load Files")
    ...
    # ----- Display labels (editable, used in every viewer title) -----
    tk.Frame(frame, height=1, bg="#333333").pack(fill="x", pady=8)
    tk.Label(frame, text="Display labels",
             font=("Helvetica", 10, "bold"), fg="#cc88ff", bg="#1a1a1a"
             ).pack(anchor="w", pady=(0, 2))
    tk.Label(frame, text="Shown in every window title (e.g. 'Pre-contrast' / "
                         "'Post-contrast', 'Baseline' / 'Follow-up').",
             font=("Helvetica", 9, "italic"), fg="#888888", bg="#1a1a1a",
    ...
    def make_label_row(parent, key, display_name, default):
        row = tk.Frame(parent, bg="#1a1a1a")
        row.pack(fill="x", pady=3)
        tk.Label(row, text=display_name, font=("Helvetica", 10),
                 fg="white", bg="#1a1a1a", width=26, anchor="w"
                 ).pack(side="left")
        var = tk.StringVar(value=default)
        entry = tk.Entry(row, textvariable=var, font=("Helvetica", 10),
                         bg="#222222", fg="#00ff88", insertbackground="white",
                         relief="flat", width=28)
        entry.pack(side="left", padx=(5, 5), ipady=2)
        label_vars[key] = var

    make_label_row(frame, "scan1_label", "Scan 1 label",     "T1w")
    make_label_row(frame, "scan2_label", "Scan 2 label",     "T1c")
    make_label_row(frame, "window_title", "Window title",    "Registration Quality")

    # Status / launch
    status_var = tk.StringVar(value="Select two scans to continue")
    ...
    def update_launch_btn():
        if paths["t1w"] and paths["t1c"]:
            ...
            missing = []
            if not paths["t1w"]: missing.append("Scan 1")
            if not paths["t1c"]: missing.append("Scan 2")
            status_var.set(f"Still need: {', '.join(missing)}")
            status_lbl.config(fg="#ff6666")

    def on_launch():
        # Copy current text from the label entries into paths;
        # fall back to defaults if a user blanks a field.
        paths["scan1_label"] = label_vars["scan1_label"].get().strip() or "T1w"
        paths["scan2_label"] = label_vars["scan2_label"].get().strip() or "T1c"
        paths["window_title"] = (label_vars["window_title"].get().strip()
...
def load_and_launch(paths):
    ...
    `paths` may also contain optional display labels:
        scan1_label, scan2_label, window_title
    which override the default "T1w" / "T1c" / "Registration Quality" strings
    used throughout the viewer titles and console messages.
    """

    scan1_label = paths.get("scan1_label") or "T1w"
    scan2_label = paths.get("scan2_label") or "T1c"
    window_title = paths.get("window_title") or "Registration Quality"

    print(f"Loading {scan1_label}: {paths['t1w']}")
    t1w_raw = load_nifti(paths["t1w"])
    print(f"Loading {scan2_label}: {paths['t1c']}")
    t1c_raw = load_nifti(paths["t1c"])

    if t1w_raw.shape != t1c_raw.shape:
        print(f"ERROR: Shape mismatch — {scan1_label} {t1w_raw.shape} "
              f"vs {scan2_label} {t1c_raw.shape}")
        sys.exit(1)
    ...
    if paths.get("labels_t1w") and paths.get("labels_t1c"):
        ...
        print(f"\nGT {scan1_label} vs GT {scan2_label} disagreement:")
    ...
    if paths.get("pred_t1w"):
        print(f"Loading {scan1_label} prediction: {paths['pred_t1w']}")
        pred_t1w_vol = load_nifti(paths["pred_t1w"]).astype(np.int32)
    if paths.get("pred_t1c"):
        print(f"Loading {scan2_label} prediction: {paths['pred_t1c']}")
    ...
    if labels_t1w_vol is not None and pred_t1w_vol is not None:
        ...
        tot = valid.sum()
        print(f"\n{scan1_label} GT vs Prediction: {dis:,}/{tot:,} "
              f"({100*dis/tot:.2f}%)")
    ...
    if labels_t1c_vol is not None and pred_t1c_vol is not None:
        ...
        tot = valid.sum()
        print(f"{scan2_label} GT vs Prediction: {dis:,}/{tot:,} "
              f"({100*dis/tot:.2f}%)")
    ...
    if pred_t1w_vol is not None and pred_t1c_vol is not None:
        ...
        tot = valid.sum()
        print(f"Pred {scan1_label} vs Pred {scan2_label}: {dis:,}/{tot:,} "
              f"({100*dis/tot:.2f}%)")

    initial_axis = paths.get("axis") or "axial"
    init_slice = paths.get("slice")
    if init_slice is None:
    ...
    run_viewer(t1w, t1c, brain_mask,
               ...
               scan1_label=scan1_label, scan2_label=scan2_label,
...
def main():
    ...
    if len(sys.argv) > 1:
        parser = argparse.ArgumentParser(
            description="Registration Viewer — compare two co-registered 3D "
                        "NIfTI volumes side by side, with optional GT label "
                        "and model-prediction overlays.",
        )
        parser.add_argument("t1w", help="Path to the first NIfTI volume "
                                        "(displayed as scan 1)")
        parser.add_argument("t1c", help="Path to the second NIfTI volume "
                                        "(displayed as scan 2)")
        parser.add_argument("--labels_t1w", default=None,
                            help="GT label map aligned to scan 1")
        parser.add_argument("--labels_t1c", default=None,
                            help="GT label map aligned to scan 2")
        parser.add_argument("--pred_t1w", default=None,
                            help="Model prediction aligned to scan 1")
        parser.add_argument("--pred_t1c", default=None,
                            help="Model prediction aligned to scan 2")
        parser.add_argument("--axis", default="axial",
                            choices=["axial", "coronal", "sagittal"])
        parser.add_argument("--slice", type=int, default=None)
        parser.add_argument("--scan1_label", default="T1w",
                            help="Display label for scan 1 in every window "
                                 "title (default: T1w)")
        parser.add_argument("--scan2_label", default="T1c",
                            help="Display label for scan 2 in every window "
                                 "title (default: T1c)")
        parser.add_argument("--window_title", default="Registration Quality",
                            help="Prefix used in the main suptitles "
                                 "(default: 'Registration Quality')")
        ...
        paths = {
            ...
            "axis": args.axis,
            "slice": args.slice,
            "scan1_label": args.scan1_label,
            "scan2_label": args.scan2_label,
